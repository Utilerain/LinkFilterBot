...
async def LinkAdd(message: types.Message):
    ...
    link = original.split("/", maxsplit=1)[0]
...
async def LinkDelete(message: types.Message):
    ...
    original = message.text.split(maxsplit=1)[1]
    original = original.replace("https://", "")
    link = original.replace(original.split("/", maxsplit=1)[1], "")

    param = cur.execute("SELECT link FROM whitelist WHERE chat_id = ? AND link = ?", (message.chat.id, link))
    if param.fetchone() is not None:
        cur.execute("DELETE link FROM whitelist WHERE chat_id = ? AND link = ?", (message.chat.id, link))
        db.commit()
        await message.reply("Link deleted from whitelist")
    else:
